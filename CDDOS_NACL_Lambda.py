...
def lambda_handler(event, context):
    ...
    def UpdateAttack_state(region,Lambda_name,state):
        client = boto3.client('lambda', region_name=region)

        Customerip = str(os.environ['customer_ip'].split())
        Customerurl = str(os.environ['customer_url'])
        Prod_Nacl = str(os.environ['prod_nacl_id'])
        Sub_id = str(os.environ['subnet_id'].split(","))
        vpc_idos = str(os.environ.get('vpc_id'))
        SD = str(os.environ['sd_ip_subnet'].split())
    ...
    if attack_status == 1 and os.environ['Attack_state'] == "0":
        print("Under Attack")
        UpdateAttack_state(Region,FunctionName,"1")
        nac_id = CreateNacl("Under_attack", vpc)
        UpdateAclEntry_Outbound(nac_id)
        UpdateAclEntry(nac_id,ip_to_add, NACL_rule_number,443,'6')
        UpdateAclEntry(nac_id,ip_to_add, NACL_rule_number,80,'6')
        UpdateAclEntry(nac_id,ip_to_add, NACL_rule_number, 22,'6')
        UpdateAclForSubnet(prod_nacl, nac_id, subnet_prod_id)



    if attack_status == 0:
        UpdateAttack_state(Region,FunctionName,"0")
        print("Peactime")
